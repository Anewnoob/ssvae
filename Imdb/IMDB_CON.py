...
def classifer(encoder_embed_input,l_a,keep_prob=0.5,reuse=False):
    with tf.variable_scope("classifier",reuse=reuse):
        ...
        return pred
def encoder(l_encoder_embed_input,l_y,keep_prob=0.5,reuse=False,training = False):
...
def get_cost_l(l_encoder_embed_input,decoder_embed_input,l_y,decoder_y,target_sequence_length,max_target_sequence_length,reuse=False):
    #encoder
    #q(a|x) q(z|axy)
    l_a,l_z, states, a_mean, a_stddev, z_mean, z_stddev  = encoder(l_encoder_embed_input,l_y,keep_prob,reuse,training)
...
def get_cost_u(u_encoder_embed_input,u_decoder_embed_input):
    ...
    #prob_y2 = tf.one_hot(tf.argmin(L_ulab,1),label_size)
    #U = -tf.reduce_sum(tf.multiply(prob_y2, prob_y), 1)
    #U = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prob_y, labels=prob_y2))
    ...
    #classifer
    prob_y  = classifer(u_encoder_embed_input,l_a,keep_prob=keep_prob,reuse=True)
    prob_y = tf.nn.softmax(prob_y)
...
def calc_kl(a_mean, a_stddev, z_mean, z_stddev,u_mean, u_stddev):
    ...
    #y_priori = tf.ones(l_y.shape, dtype=tf.float32)/tf.to_float(l_y.shape[1])
    #latent_loss_p0y = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=y_priori, labels=l_y))
    #latent_cost_p0y = tf.reduce_mean(latent_loss_p0y)
...
pred=classifer(l_encoder_embed_input,l_a)
...
def train_model():
    ...
    with tf.Session() as sess:
        sess.run(initial)
        #saver.restore(sess, './temp/imdb_2500_con_lvae.pkt')
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainS) // batch_size:
                ...
                p_y,ulab,pred_batch,c_pred,op,batch_cost,c_cost,l_cost,u_cost=sess.run([prob_y,L_ulab,pred,correct_pred,optimizer,cost,cost_c,cost_l,cost_u],
            ...
            if TEST_acc1 > max_acc:
                max_acc = TEST_acc1
                model_path = './temp/imdb_%d_con_sigmoid_lvae.pkt'%Train_Size
...
def test_model(sess,testS,testL,epoch):
    ...
    while step < len(testS) // batch_size:  #
        ...
        c_pred,pred_batch=sess.run([correct_pred,pred],feed_dict={l_encoder_embed_input:sources_batch, l_y: batch_y,
                                                                           keep_prob: 1.0,l_decoder_embed_input: input_batch,training:False,
